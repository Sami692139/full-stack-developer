...
const getSingleCustomer = async (customerId) => {
  try {
    // SQL query to retrieve customer details by customer ID by joining customer_identifier and customer_info tables
    const query = `
      SELECT ci.customer_id, ci.customer_email, ci.customer_phone_number, ci.customer_added_date, 
             ci.customer_hash, c.customer_first_name, c.customer_last_name, 
             c.active_customer_status
    ...
    // Execute the query to fetch the customer record
    ...
    // Return the first (and only) customer found
    return rows[0];
  } catch (error) {
